...
export class GameRoom {
  ...
  private updateTackle(r:MatchRuntime){const t=r.tackle;if(!t)return;const now=Date.now();if(t.impactAt){if(now>t.timeoutAt)this.startPenalty(r,"regular",t.kickerId,t.goalieId);return;}if(this.players.get(t.kickerId)?.isBot){t.kickerX=clamp(t.kickerX+(Math.random()-.4)*14,150,590);t.kickerY=clamp(t.kickerY+(Math.random()-.5)*14,180,570);}const dx=t.kickerX-t.goalieX,dy=t.kickerY-t.goalieY,dist=Math.hypot(dx,dy)||1;const chaseSpeed=dist<360?54:38;t.goalieX+=(dx/dist)*chaseSpeed;t.goalieY+=(dy/dist)*chaseSpeed;const ridiculousHitRadius=145;if(dist<ridiculousHitRadius||now>t.timeoutAt){t.impactAt=now;t.timeoutAt=now+6500;t.goalieX=t.kickerX+72;t.goalieY=t.kickerY+18;r.message=`${this.nameOf(t.goalieId)} launches a ${t.tackleStyle === "flyingKick" ? "flying kick" : t.tackleStyle === "spinKick" ? "spin kick" : t.tackleText}! ${this.nameOf(t.kickerId)} begins a world-class flop.`;}}
  private handleMove(c:ClientSocket,data:unknown){const r=this.runtimeForPlayer(c.sessionId);if(!r||r.phase!=="tackle"||!r.tackle||c.sessionId!==r.tackle.kickerId)return;const d=data as {dx?:number;dy?:number};r.tackle.kickerX=clamp(r.tackle.kickerX+clamp(Number(d.dx??0),-1,1)*18,150,590);r.tackle.kickerY=clamp(r.tackle.kickerY+clamp(Number(d.dy??0),-1,1)*18,180,570);}
