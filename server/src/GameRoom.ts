...
export class GameRoom {
  ...
  private updateTackle(r:MatchRuntime){
    ...
    if(this.players.get(t.kickerId)?.isBot){
      t.kickerX=clamp(t.kickerX+(Math.random()-.45)*16,150,590);
      t.kickerY=clamp(t.kickerY+(Math.random()-.5)*16,180,570);
    }
    const goalieIsBot=!!this.players.get(t.goalieId)?.isBot;
    let dx=t.kickerX-t.goalieX,dy=t.kickerY-t.goalieY,dist=Math.hypot(dx,dy)||1;
    // Bots chase normally. Human defenders use their own controls, with a dramatic
    // server-assisted lunge in the final second so nobody can survive past 5 seconds.
    if(goalieIsBot){
      const chaseSpeed=elapsed>3600?58:34;
      t.goalieX=clamp(t.goalieX+(dx/dist)*chaseSpeed,180,1140);
      t.goalieY=clamp(t.goalieY+(dy/dist)*chaseSpeed,170,580);
    }else if(elapsed>4000){
      const lungeSpeed=78;
      t.goalieX=clamp(t.goalieX+(dx/dist)*lungeSpeed,180,1140);
      t.goalieY=clamp(t.goalieY+(dy/dist)*lungeSpeed,170,580);
    }
    dx=t.kickerX-t.goalieX;dy=t.kickerY-t.goalieY;dist=Math.hypot(dx,dy)||1;
    const hitRadius=elapsed<1800?72:elapsed<4000?96:175;
  ...
  private handleMove(c:ClientSocket,data:unknown){
    const r=this.runtimeForPlayer(c.sessionId);if(!r||r.phase!=="tackle"||!r.tackle||r.tackle.impactAt)return;
    const t=r.tackle;if(c.sessionId!==t.kickerId&&c.sessionId!==t.goalieId)return;
    const d=data as {dx?:number;dy?:number};let dx=clamp(Number(d.dx??0),-1,1),dy=clamp(Number(d.dy??0),-1,1);
    const magnitude=Math.hypot(dx,dy);if(magnitude>1){dx/=magnitude;dy/=magnitude;}
    if(c.sessionId===t.kickerId){
      const speed=14;
      t.kickerX=clamp(t.kickerX+dx*speed,150,590);
      t.kickerY=clamp(t.kickerY+dy*speed,180,570);
    }else{
      const speed=23;
      t.goalieX=clamp(t.goalieX+dx*speed,180,1140);
